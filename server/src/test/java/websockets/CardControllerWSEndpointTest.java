...
public class CardControllerWSEndpointTest {
    ...
    void deleteMessage() throws Exception {
        ...
        when(cardRepository.findById(0L)).thenReturn(Optional.empty());
        ...
        doNothing()
                .when(cardRepository).moveAllCardsHigherThanIndexDown(0L, 0);
        ...
        session.subscribe("/topic/cards/delete" + 0L, new StompFrameHandler() {
            @Override
            public Type getPayloadType(StompHeaders headers) {
                return Long.class;
            }
        ...
        verify(cardRepository, times(1)).findById(0L);
        verifyNoMoreInteractions(cardRepository);
    }
    ...
    void moveCardMessage1() throws Exception {
        ...
        session.subscribe("/topic/cards/move" + 0L, new StompFrameHandler() {
            @Override
            public Type getPayloadType(StompHeaders headers) {
        ...
        await()
            .atMost(1, SECONDS)
            .untilAsserted(() -> {
                Assertions.assertNull(blockingQueue.peek());
            });
    ...
    void moveCardMessage3() throws Exception {
        ...
        session.send("/app/cards/move" + 0L, message);
